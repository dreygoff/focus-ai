...
class RealSessionRepository(
    ...
    override fun observeSessions(startDate: Long, endDate: Long): Flow<List<app.focus.domain.model.Session>> = sessionDao.observeSessionsInRange(startDate, endDate).mapNotNull { it.toDomain() }
...
class RealScheduleRepository(
    ...
    override fun observeSchedules(): Flow<List<app.focus.domain.model.Schedule>> = scheduleDao.observeSchedules().mapNotNull { it.toDomain() }
...
class RealAccessWindowRepository(
    ...
    override fun observeActiveWindows(sessionId: String): Flow<Map<String, AccessWindow>> {
        val now = System.currentTimeMillis()
        return accessWindowDao.observeActiveWindows(sessionId).mapNotNull { e -> e.toDomain().takeIf { w -> w.expiresAt > now } }.associateBy { it.packageName }
    }
...
class RealEventLogRepository(
    ...
    override fun observeEventsForSession(sessionId: String): Flow<List<app.focus.domain.model.EventLog>> = eventLogDao.observeEventsForSession(sessionId).mapNotNull { it.toDomain() }
