...
class ProtoUserSettingsRepository(
    ...
    override fun getTheme(): Flow<Theme> = dataStore.data.map { proto -> proto.theme.toDomain() }

    override fun getDynamicColorEnabled(): Flow<Boolean> = dataStore.data.map { it.dynamicColor }

    override fun getLanguageTag(): Flow<String> = dataStore.data.map { it.languageTag.ifBlank { "ru" } }

    override suspend fun isOnboardingCompleted(): Boolean = dataStore.data.first().onboardingCompleted
    ...
    override suspend fun isAccessibilityDisclosureAccepted(): Boolean = dataStore.data.first().accessibilityDisclosureAccepted
    ...
    override suspend fun updateLastUsedProfile(
        profileId: String,
        durationMinutes: Int,
    ) {
        dataStore.updateData { current ->
    ...
    override fun getBlockVibrationEnabled(): Flow<Boolean> = dataStore.data.map { it.blockVibration }

    override fun getBlockSoundEnabled(): Flow<Boolean> = dataStore.data.map { it.blockSound }

    override fun getQuotesEnabled(): Flow<Boolean> = dataStore.data.map { it.quotesEnabled }
...
private object UserSettingsSerializer : Serializer<UserSettingsProto> {
    ...
    override suspend fun readFrom(input: InputStream): UserSettingsProto = UserSettingsProto.parseFrom(input)
...
private fun UserSettingsProto.Theme.toDomain(): Theme =
    when (this) {
...
private fun Theme.toProto(): UserSettingsProto.Theme =
    when (this) {
